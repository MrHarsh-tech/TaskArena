  ...
  const submitAttempt = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        alert(data.error || "An error occurred while submitting your attempt.");
        return;
      }
      // Normalize percentage to avoid NaN rendering
      if (data && (isNaN(data.percentage) || data.percentage == null)) {
        data.percentage = 0;
      }
      setResult(data);
