  ...
  try {
    ...
    const gradedAnswers = answers.map((answer) => {
      ...
      if (question.questionType === "FILL_IN_BLANK") {
        ...
        if (userAnswer) {
          const acceptedList = question.acceptedAnswers || [];
          isCorrect = acceptedList.some((accepted) => {
    ...
    try {
      ...
      throw new Error("Failed at attempt.create: " + (e instanceof Error ? e.message : String(e)));
    }
    ...
    const sumPercentages = allAttempts.reduce((sum, a) => sum + (isNaN(a.percentage) ? 0 : a.percentage), 0);
    ...
    try {
      ...
      throw new Error("Failed at progress.upsert: " + (e instanceof Error ? e.message : String(e)));
    }
    ...
    try {
      ...
      throw new Error("Failed at user.update: " + (e instanceof Error ? e.message : String(e)));
    }

    return NextResponse.json(
      {
        ...attempt,
        // Explicitly include numeric fields to prevent NaN serialization issues
        score: attempt.score,
        totalQuestions: attempt.totalQuestions,
        percentage: isNaN(attempt.percentage) ? percentage : attempt.percentage,
        xpEarned,
    ...
    return NextResponse.json(
      { error: "Failed to submit attempt: " + (error instanceof Error ? error.message : String(error)) },
      { status: 500 }
    );
  }
}
